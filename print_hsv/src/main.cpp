...
static void setupGUI() {
	...
	setMouseCallback( winIn, onMouse, 0 );
...
static void printHSV(const Mat img) {
	if (isHsvPrinted) return;
	...
	int x = mousePos.x;
	int y = mousePos.y;
	uchar h = hsvImg.at<cv::Vec3b>(y,x)[0];
	uchar s = hsvImg.at<cv::Vec3b>(y,x)[1];
	uchar v = hsvImg.at<cv::Vec3b>(y,x)[2];

	printf("H: %d, S:%d, V:%d in (%d, %d)", h, s, v, x, y);
	isHsvPrinted = true;
